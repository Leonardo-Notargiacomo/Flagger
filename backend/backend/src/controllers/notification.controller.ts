...
export class NotificationController {
   ...
   * Test endpoint to manually trigger notifications for a user
  ...
  async triggerTestNotification(
      ...
      content: {
        'application/json': {
          schema: {
            ...
            properties: {
              userId: {type: 'number'},
              type: {type: 'string', enum: ['doing_well', 'skipping']},
            },
          },
        },
      },
    ...
    if (process.env.NODE_ENV === 'production') {
      throw new HttpErrors.Forbidden('Test endpoints not available in production');
    }

    console.log(`[NotificationController] Test notification for user ${data.userId}, type: ${data.type}`);

    try {
      // Get targets based on type
      const targets = data.type === 'doing_well'
        ? await this.notificationTriggerService.getUsersDoingWell()
        : await this.notificationTriggerService.getUsersSkipping();

      // Find the specific user
      const userTarget = targets.find(t => t.userId === data.userId);

      if (!userTarget) {
        return {
          success: false,
          message: `User ${data.userId} not eligible for ${data.type} notification`,
          allTargets: targets.map(t => t.userId),
        };
      }
      ...
      console.error('[NotificationController] Error triggering test notification:', error);
      throw new HttpErrors.InternalServerError('Failed to trigger test notification');
    }
  }
}
